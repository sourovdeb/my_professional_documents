...
Palette + layout match Episodes 1 & 2 (Ocean Gradient).
"""
...
para(tf, "It’s just the thing you type (What & Why)", 34, NAVY, bold=True, first=True)
tf = box(s, 0.9, 1.7, 11.6, 5.2)
para(tf, "A prompt = your instruction to the AI, in plain words", 24, DEEP_BLUE, bold=True, first=True, space=14)
for t in [
    "▪  If you’ve typed into a Google search box, you’ve written a prompt",
    "▪  Same move as Word’s Find box or texting “bring milk” — just bigger",
    "▪  Vague prompt → vague answer.  Specific prompt → specific answer",
    "▪  The model can’t read your mind — only your words",
]:
    para(tf, t, 20, INK, space=10)
para(tf, "→ You bring the intent. It brings the typing.", 20, TEAL, bold=True, space=6)

# ---------- Slide 3: How it works ----------
s = prs.slides.add_slide(BLANK); bg(s, ICE); accent_bar(s)
tf = box(s, 0.7, 0.5, 12, 1.0)
para(tf, "How does it work?  The first domino", 34, NAVY, bold=True, first=True)
tf = box(s, 0.9, 1.7, 11.6, 5.2)
para(tf, "Your prompt is the OPENING the model continues.", 24, DEEP_BLUE, bold=True, first=True, space=14)
for t in [
    "▪  Remember Episode 2 — the model just guesses the next word",
    '▪  “Write a haiku about rain” → the likely continuation IS a haiku',
    "▪  Place the first domino (the prompt); the guesses fall that way",
    "▪  Change the wording → change the whole line that follows",
]:
    para(tf, t, 20, INK, space=10)
para(tf, "That’s why “Explain this” and “Explain this to a 10-year-old in 3 sentences” differ so much.",
...
tf = box(s, 0.9, 1.7, 11.6, 5.2)
para(tf, "Open console.mistral.ai → Le Chat → the wide box at the bottom", 22, ICE, bold=True, first=True, space=14)
for t in [
    "▪  That message box is where your prompt lives — type there, hit send",
    "▪  Lazy prompt: “email about the picnic” → generic result",
    "▪  Clear prompt: who it’s for + what + length + tone → sharp result",
    "▪  A separate “System Prompt” field = a standing, permanent prompt",
]:
...
tf = box(s, 0.9, 1.7, 11.6, 5.2)
for t in [
    "Step 1  —  In Le Chat, type “email about the picnic” and send",
    "Step 2  —  Now type a specific version: who, what, length, tone — send",
    "Step 3  —  Read both replies side by side; spot the difference",
    "Bonus   —  Keep the good prompt; you’ll reuse it for years",
]:
    para(tf, t, 21, INK, space=12)
para(tf, "Takeaway: brief the AI like a smart new assistant — who, what, how long, what format.",
     21, TEAL, bold=True, space=8)
para(tf, "Next episode →  Token: the tiles a model chops your words into.", 18, MUTED, space=4)
